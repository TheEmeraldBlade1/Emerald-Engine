...
class FreeplayState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
		...
		else #end if (accepted)
		{
			...
			#if MODS_ALLOWED
			if(!sys.FileSystem.exists(Paths.modsJson(songLowercase + '/' + songLowercase + "-hard")) && !sys.FileSystem.exists(Paths.json(songLowercase + '/' + songLowercase + "-hard"))) {
			#else
			if(!OpenFlAssets.exists(Paths.json(songLowercase + '/' + songLowercase + "-hard"))) {
			#end
				poop = songLowercase;
				curDifficulty = 1;
