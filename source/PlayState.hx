...
import uiAssets.ScoreRatingsText;
...
class PlayState extends MusicBeatState
{
	...
	var cornerMark:FlxText;

	private var accuracy:Float = 0.00;
	private var newAccuracy:Float = 0.00;
	...
	var rankingsGroup:FlxTypedGroup<FlxText>;
	var timeTxt:FlxText;
	var scoreTxtTween:FlxTween;
	var scoreTxtTween2:FlxTween;
	var scoreTxtTween3:FlxTween;
	var scoreTxtTween4:FlxTween;
	...
	override public function create()
	{
		...
		strumLine = new FlxSprite(ClientPrefs.middleScroll == 1 ? STRUM_X_MIDDLESCROLL : STRUM_X, 50).makeGraphic(FlxG.width, 10);
		...
		timeBarBG.x = STRUM_X + (FlxG.width / 2) - 248;
		timeBarBG.y = 0;
		...
		scoreTxt.setFormat(ScoreRatingsText.instance.getFont(), 18, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.hideHudScore;
		scoreTxt.text = ScoreRatingsText.instance.firstScore();
		add(scoreTxt);
		...
		cornerMark = new FlxText(0, 0, 0, 'EMERALD ENGINE v' + Main.stringVersion + '\n');
	...
	public function startCountdown():Void
	{
		...
		if(ret != FunkinLua.Function_Stop) {
			...
			for (i in 0...opponentStrums.length) {
				...
				if(ClientPrefs.hideEnemyNotes) opponentStrums.members[i].visible = false;
			}
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
			{
				...
				notes.forEachAlive(function(note:Note) {
					...
					if(ClientPrefs.middleScroll == 1 && !note.mustPress) {
						note.alpha *= 0.5;
					}
				});
	...
	function startSong():Void
	{
		...
		if (PublicVariables.transParentText.contains(SONG.song)){
			FlxTween.tween(timeTxt, {alpha: 0.50}, 0.5, {ease: FlxEase.circOut});
		}else{
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (OpenFlAssets.exists(file)) {
			...
			for (section in eventsData)
			{
				...
				{
					if(songNotes[1] < 0) {
						eventNotes.push([songNotes[0], songNotes[1], songNotes[2], songNotes[3], songNotes[4], songNotes[5], songNotes[6], songNotes[7], songNotes[8]]);
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				if(songNotes[1] > -1) { //Real notes
					...
					if (ClientPrefs.iconColorNotes && gottaHitNote){
						swagNote.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
					}

					if (ClientPrefs.iconColorNotes && !gottaHitNote){
						swagNote.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
					}
					...
					if(floorSus > 0) {
						for (susNote in 0...floorSus+1)
						{
							...
							if (ClientPrefs.iconColorNotes && gottaHitNote){
								sustainNote.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
							}
		
							if (ClientPrefs.iconColorNotes && !gottaHitNote){
								sustainNote.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
							}
							...
							else if(ClientPrefs.middleScroll == 1)
								{
									sustainNote.x += 310;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player < 1 && ClientPrefs.middleScroll == 1) targetAlpha = 0.35;

			var babyArrow:StrumNote = new StrumNote(ClientPrefs.middleScroll == 1 ? STRUM_X_MIDDLESCROLL : STRUM_X, strumLine.y, i, player);
			if (!isStoryMode)
			{
				//babyArrow.y -= 10;
				babyArrow.alpha = 0;
			...
			if (player == 1)
			{
				...
				if (ClientPrefs.iconColorNotes){
					babyArrow.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
				}
				if (ClientPrefs.middleScroll == 2 || PublicVariables.noteFlip.contains(SONG.song) && ClientPrefs.middleScroll == 0 && !PublicVariables.noteFlipApart.contains(SONG.song)){
					babyArrow.x -= 732;
					babyArrow.x += STRUM_X + 74;
				}
				if (PublicVariables.noteFlip.contains(SONG.song) && ClientPrefs.middleScroll == 0 && PublicVariables.noteFlipApart.contains(SONG.song) || ClientPrefs.middleScroll == 3){
					babyArrow.x -= 732;
				}
				playerStrums.forEach(function(spr:FlxSprite){
					trace("PlayerStrums: x:" + spr.x + ",y:" + spr.y);
				});
			}
			else
			{
				...
				if (ClientPrefs.iconColorNotes){
					babyArrow.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
				}
				if (ClientPrefs.middleScroll == 2 || PublicVariables.noteFlip.contains(SONG.song) && ClientPrefs.middleScroll == 0 && !PublicVariables.noteFlipApart.contains(SONG.song)){
					babyArrow.x = 732;
					babyArrow.x -= STRUM_X + 24;
				}
				if (PublicVariables.noteFlip.contains(SONG.song) && ClientPrefs.middleScroll == 0 && PublicVariables.noteFlipApart.contains(SONG.song) || ClientPrefs.middleScroll == 3){
					babyArrow.x = 732 + STRUM_X + 6;
				}
				if (ClientPrefs.hideEnemyNotes || PublicVariables.hideEnemyNotesSongs.contains(SONG.song)){
				...
				opponentStrums.forEach(function(spr:FlxSprite){
					trace("OpponentStrums: x:" + spr.x + ",y:" + spr.y);
				});
			}
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.maxMisses){
			health = 1;
			if (songMisses > SONG.maxMissesValue){
				health = 0;
			}
		...
		if (songMisses == 0 && shits == 0 && bads == 0 && goods == 0){
		...
		if (FlxG.keys.justPressed.FIVE && !endingSong && !inCutscene) {
			...
			cancelFadeTween();
			CustomFadeTransition.nextCamera = camOther;
			MusicBeatState.switchState(new CharacterEditorState(SONG.player3));
		}
		...
		if (generatedMusic)
		{
			...
			{
				if(!daNote.mustPress && ClientPrefs.hideEnemyNotes)
				{
					daNote.active = true;
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					if(daNote.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
						...
						if(daNote.noteType == 'GF Sing') {
							...
							gf.holdTimer = 0;
						} else if(daNote.noteType == 'GF And Character Sing') {
							gf.playAnim(animToPlay + altAnim, true);
							gf.holdTimer = 0;
					...
					var time:Float = ClientPrefs.cpuNoteStrumTimer;
					...
					if (!daNote.isSustainNote && ClientPrefs.noteSplashes2){
						spawnNoteSplashOnNoteEnemy(daNote, ClientPrefs.hideEnemyNotes);
					}

					if (SONG.hpDrain){
						if (health > 0.075){
							health -= SONG.hpDrainValue;
						}
					}
				...
				if (doKill)
				{
					...
					}else if (daNote.mustPress && cpuControlled &&!daNote.ignoreNote && !endingSong && (daNote.tooLate || !daNote.wasGoodHit)){
						goodNoteHit(daNote);
					}
		...
		if (Main.debugBuild){
			if (FlxG.keys.justPressed.TAB){
				boyfriendGroup.visible = !boyfriendGroup.visible;
				gfGroup.visible = !gfGroup.visible;
				dadGroup.visible = !dadGroup.visible;
				if (!ClientPrefs.hideHudIconp1){
					iconP1.visible = !iconP1.visible;
				}
				if (!ClientPrefs.hideHudIconp2){
	...
	public function checkEventNote() {
		while(eventNotes.length > 0) {
			...
			if(eventNotes[0][5] != null)
				value3 = eventNotes[0][5];

			var value4:String = '';
			if(eventNotes[0][6] != null)
				value4 = eventNotes[0][6];

			var value5:String = '';
			if(eventNotes[0][7] != null)
				value5 = eventNotes[0][7];

			var value6:String = '';
			if(eventNotes[0][8] != null)
				value6 = eventNotes[0][8];

			triggerEventNote(eventNotes[0][2], value1, value2, value3, value4, value5, value6);
			eventNotes.shift();
		}
	}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, value3:String, value4:String, value5:String, value6:String) {
		var floatValue1:Float = Std.parseFloat(value1);
		var floatValue2:Float = Std.parseFloat(value2);
		var floatValue3:Float = Std.parseFloat(value3);
		var floatValue4:Float = Std.parseFloat(value4);
		var floatValue5:Float = Std.parseFloat(value5);
		var floatValue6:Float = Std.parseFloat(value6);
		var intValue1:Int = Std.parseInt(value1);
		var intValue2:Int = Std.parseInt(value2);
		var intValue3:Int = Std.parseInt(value3);
		var intValue4:Int = Std.parseInt(value4);
		var intValue5:Int = Std.parseInt(value5);
		var intValue6:Int = Std.parseInt(value6);
		switch(eventName) {
			case 'Change Scroll Speed':
				setSongScrollSpeed(floatValue1);
			case 'Hey!':
				var value:Int = 2;
			...
			case 'Change Character':
				...
				switch(value1.toLowerCase()) {
					case 'BF' | '0' | 'bf' | 'boyfriend' | 'Boyfriend':
					...
					case 'GF' | '2' | 'gf' | 'girlfriend' | 'Girlfriend':
		...
		callOnLuas('onEvent', [eventName, value1, value2, value3, value4, value5, value6]);
	}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			if (SONG.validScore)
			{
				...
				Highscore.saveScore(SONG.song, trueScore, storyDifficulty, percent);
				#end
			}

			if (isStoryMode)
			{
				campaignScore += trueScore;
	...
	public function ratingFCCall(){
		if (songMisses <= 0 && sicks == 0 && goods == 0 && bads == 0 && shits == 0){ratingFC = " [PFC]";}
		else if (songMisses <= 0 && goods == 0 && bads == 0 && shits == 0){ratingFC = " [SFC]";}
		else if (songMisses <= 0 && bads == 0 && shits == 0){ratingFC = " [GFC]";}
		else if (songMisses <= 0){ratingFC = " [FC]";}
		else {ratingFC = "";}
		return ratingFC;
	}
	...
	function tweenTxtFunction(){
		...
		if(scoreTxtTween4 != null) {
			scoreTxtTween4.cancel();
		}
		scoreTxt.scale.x = 1.1; judgementTxt.scale.x = 1.1; botplayTxt.scale.x = 1.1; ratingTxt.scale.x = 1.1;
		scoreTxt.scale.y = 1.1; judgementTxt.scale.y = 1.1; botplayTxt.scale.y = 1.1; ratingTxt.scale.y = 1.1;
		scoreTxtTween = FlxTween.tween(scoreTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				scoreTxtTween = null;
			}
		});
		scoreTxtTween2 = FlxTween.tween(judgementTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				scoreTxtTween2 = null;
			}
		});
		scoreTxtTween3 = FlxTween.tween(botplayTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				scoreTxtTween3 = null;
			}
		});
		scoreTxtTween4 = FlxTween.tween(ratingTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: function(twn:FlxTween) {
				scoreTxtTween4 = null;
			}
		});
	...
	private function popUpScore(note:Note = null, isMiss:Bool):Void
	{
		...
		if (!cpuControlled){
			if (isMiss){
				...
				score = -500;
				songMisses += 1;
				EmeraldEngineAccuracy.ratingUpdate(0,0,0,0);
				newAccuracy -= rng.float(1.25,2.25);
				...
				if (noteDiff > Conductor.safeZoneOffset * 0.8){
					...
					newAccuracy -= rng.float(1.25,2.25);
					isMiss = true;
					if (!SONG.cheatingNotes && !SONG.unfairnessNotes && !SONG.randomNotes){
				...
				else if (noteDiff > Conductor.safeZoneOffset * 0.7)
					{
						...
						score = 50;
						shits += 1;
						EmeraldEngineAccuracy.ratingUpdate(rng.int(0,5),rng.int(0,5),0,0);
						newAccuracy -= rng.float(0.85,1);
						health += note.hitHealth;
					}
					else if (noteDiff > Conductor.safeZoneOffset * 0.6)
					{
						...
						score = 100;
						bads += 1;
						EmeraldEngineAccuracy.ratingUpdate(rng.int(15,23),rng.int(5,14),0,0);
						newAccuracy -= rng.float(0.25,0.75);
						health += note.hitHealth;
					}
					else if (noteDiff > Conductor.safeZoneOffset * 0.5)
					{
						...
						score = 200;
						goods += 1;
						EmeraldEngineAccuracy.ratingUpdate(rng.int(34,46),rng.int(0,55),0,0);
						newAccuracy += rng.float(0.25,0.35);
						health += note.hitHealth;
					}
					else if (noteDiff > Conductor.safeZoneOffset * 0.4)
					{
						...
						totalNotesHit += 1;
						if (songMisses == 0 && shits == 0 && bads == 0 && goods == 0){
							EmeraldEngineAccuracy.ratingUpdate(rng.int(100,100),rng.int(99,99),0,0);
						}else{
							EmeraldEngineAccuracy.ratingUpdate(rng.int(50,75),rng.int(23,75),0,0);
						}
						newAccuracy += rng.float(0.75,0.85);
						health += note.hitHealth;
					}
					else
					{
						...
						if (songMisses == 0 && shits == 0 && bads == 0 && goods == 0){
							...
							EmeraldEngineAccuracy.ratingUpdate(rng.int(85,99),rng.int(60,99),0,0);		
						}
						newAccuracy += rng.float(1.25,2.50);
						health += note.hitHealth;
					}
			}
		}else{
		...
		if(daRating == 'PERFECT!!!' && !note.noteSplashDisabled && !isMiss)
		{
			perfectSpawnNoteSplashOnNote(note, false);
		}

		if(daRating == 'Sick!!' && !note.noteSplashDisabled && !ClientPrefs.disableSickNoteSplashes && !isMiss)
		{
		...
		var scaleRating:Float = 0.5;

		if (ratingPath == 'perfect'){
			rating.loadGraphic(Paths.image("ratingStuff/perfect/" + ratingPath));
		}else if (ratingPath == 'sick'){
			rating.loadGraphic(Paths.image("ratingStuff/sick/" + ratingPath));
		}else if (isMiss){
			rating.loadGraphic(Paths.image("ratingStuff/miss/" + ratingPath));
		}else{
			rating.loadGraphic(Paths.image("ratingStuff/normal/" + ratingPath));
		}
		rating.screenCenter();
		...
		rating.visible = !FlxG.save.data.hideJudgementRatings;

		var comboSpr:FlxSprite;
		if (ratingPath == 'perfect'){
			comboSpr = new FlxSprite().loadGraphic(Paths.image("ratingStuff/perfect/combo"));
		}else if (ratingPath == 'sick'){
			comboSpr = new FlxSprite().loadGraphic(Paths.image("ratingStuff/sick/combo"));
		}else if (isMiss){
			comboSpr = new FlxSprite().loadGraphic(Paths.image("ratingStuff/miss/comboBreak"));
		}else{
			comboSpr = new FlxSprite().loadGraphic(Paths.image("ratingStuff/normal/combo"));
		}
		if (isMiss){
		...
		comboSpr.visible = !FlxG.save.data.hideJudgementRatings;
		...
		for (i in seperatedScore)
		{
			...
			if (ratingPath == 'perfect'){
				numScore = new FlxSprite().loadGraphic(Paths.image('ratingStuff/nums/perfect/num' + Std.int(i)));
			}else if (ratingPath == 'sick'){
				numScore = new FlxSprite().loadGraphic(Paths.image('ratingStuff/nums/sick/num' + Std.int(i)));
			}else{
				numScore = new FlxSprite().loadGraphic(Paths.image('ratingStuff/nums/normal/num' + Std.int(i)));
			}
			...
			numScore.visible = !FlxG.save.data.hideJudgementRatings;
	...
	function noteMissAnim(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
			...
			if(!note.noAnimation) {
...

				{
					if(note.noteType == 'GF Sing') {
						...
						gf.holdTimer = 0;
					} else if(note.noteType == 'GF And Character Sing') {
						gf.playAnim(animToPlay + daAlt, true);
						gf.holdTimer = 0;
						boyfriend.playAnim(animToPlay + daAlt, true);
	...
	function spawnNoteSplashOnNote(note:Note, invisible:Bool) {
		if (!invisible){
	...
	function perfectSpawnNoteSplashOnNote(note:Note, invisible:Bool) {
		if (!invisible){
			if(ClientPrefs.noteSplashes && note != null) {
				var strum:StrumNote = playerStrums.members[note.noteData];
				if(strum != null) {
					if (ClientPrefs.disablePerfectNoteSplashes){
						spawnNoteSplash(strum.x, strum.y, note.noteData, note);
					}else{
						spawnNoteSplashPerfect(strum.x, strum.y, note.noteData, note);
					}
				}
			}
		}
	...
	function spawnNoteSplashOnNoteEnemy(note:Note, invisible:Bool) {
		if (!invisible){
			if(ClientPrefs.noteSplashes && note != null) {
	...
	public function spawnNoteSplash(x:Float, y:Float, data:Int, ?note:Note = null) {
		...
		if (ClientPrefs.iconColorNotes && note.mustPress){
			splash.color = FlxColor.fromRGB(boyfriend.healthColorArray[0], boyfriend.healthColorArray[1], boyfriend.healthColorArray[2]);
		}if (ClientPrefs.iconColorNotes && !note.mustPress){
			splash.color = FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]);
		}
		grpNoteSplashes.add(splash);
	}

	public function spawnNoteSplashPerfect(x:Float, y:Float, data:Int, ?note:Note = null) {
		NoteSplash.isPerfectNote = true;
		var skin:String = 'noteSplashesPerfect';
		if(PlayState.SONG.splashSkin != null && PlayState.SONG.splashSkin.length > 0) skin = PlayState.SONG.splashSkin;
	...
	function StrumPlayPressAnim(isDad:Bool, id:Int, time:Float) {
		var spr:StrumNote = null;
		if(isDad) {
			spr = strumLineNotes.members[id];
		} else {
	...
	public function setSongScrollSpeed(speedValue:Float){
		SONG.speed = speedValue;
		trace("Speed Value: " + speedValue);
	}
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop) {
			ratingPercent = songScore / ((songHits + songMisses - ghostMisses) * 500);
